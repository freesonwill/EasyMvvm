...
class PersonalInfoAdapter :
    BaseAdapter<SystemAvatarBean, PersonalInfoItemViewHolder, ItemPersonalInfoBinding>(
        PersonalInfoCompare()
    ) {
    private var isUpAvatar = true
    private var selectedPosition: Int = -1
    private var onItemClickListener: ((SystemAvatarBean) -> Unit)? = null
    override fun convertPlus(
        holder: PersonalInfoItemViewHolder,
        binding: ItemPersonalInfoBinding,
        position: Int
    ) {
    ...
    fun setIsUpAvatar(isUpAvatar: Boolean) {
