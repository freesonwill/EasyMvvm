...
@KoinViewModel
class MatchListViewModelV2 : BaseMatchViewModel<MatchListRepository>() {
            ...
            repository.observeMatchChange(_playType, _tournamentIdList)
                ...
                .collect { refs ->
                    ...
                    "Collect observeMatchChange start playType = $_playType, sportId = ${_sportId} tournament = $_tournamentIdList selectedDate = $selectedDate".logi(
                        this@MatchListViewModelV2::class.java.simpleName
                    )
                    val currentDateRefs = refs.filter { it.date == selectedDate }
                    if (currentDateRefs.isEmpty()) {
                        "Collect observeMatchChange TournamentMatchRef is NULL!!".logi(this@MatchListViewModelV2::class.java.simpleName)
                        return@collect
                    }
                    processObserveMatchList(currentDateRefs)
                }
    ...
    private suspend fun processObserveMatchList(currentDateRefs: List<TournamentMatchRef>) {
        ...
        "Collect observeMatchChange result：${list.map { it.match.matchId }}".logi(this@MatchListViewModelV2::class.java.simpleName)
