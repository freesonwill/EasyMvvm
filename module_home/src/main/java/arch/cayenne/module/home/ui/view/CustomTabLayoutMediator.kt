...
class CustomTabLayoutMediator(
    ...
    private fun smoothScrollToTab(position: Int) {
        ...
        val targetChild = slidingTabStrip.getChildAt(position) ?: return

        // 計算目標滾動位置（將 tab 置中）
        val targetChildCenter = targetChild.left + targetChild.width / 2
        val tabLayoutCenter = tabLayout.width / 2
        val targetScrollX = (targetChildCenter - tabLayoutCenter).toFloat()

        // 限制滾動範圍
        val maxScrollX = max(0, slidingTabStrip.width - tabLayout.width)
        val clampedScrollX = targetScrollX.coerceIn(0f, maxScrollX.toFloat())

        val currentScrollX = tabLayout.scrollX.toFloat()

        // 如果距離很小，直接跳轉
        if (abs(currentScrollX - clampedScrollX) < 2) {
            tabLayout.scrollTo(clampedScrollX.toInt(), 0)
            return
        }

        // 使用 ValueAnimator 平滑滾動
        ValueAnimator.ofFloat(currentScrollX, clampedScrollX).apply {
            duration = 250L
            interpolator = FastOutSlowInInterpolator()
            addUpdateListener { animator ->
                val animatedValue = animator.animatedValue as Float
                tabLayout.scrollTo(animatedValue.toInt(), 0)
            }
            start()
        }
    }
