package com.luxury.lib_base.network.bean
import androidx.lifecycle.MutableLiveData
import com.luxury.lib_base.bean.AppException
import com.luxury.lib_base.bean.ExceptionHandle
    ...
    value = when {
        result.isSucces() -> {
           ...
           /* if (result.getResponseCode()==10){
                try {
                    val clazz = Class.forName("com.xcjh.commonview.util.LoginAndOutUtil")
                    val obj = clazz.newInstance() as Any //实例化LoginAndOutUtil
                    clazz.getDeclaredMethod("logoutClick", Context::class.java).invoke(obj,
