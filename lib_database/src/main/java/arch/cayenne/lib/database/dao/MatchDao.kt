...
abstract class MatchDao : BaseDao<MatchBean>() {
    ...
    @Query(
        "SELECT * " +
                "FROM MatchBean bean " +
                "INNER JOIN EarlyTournamentMatchRef ref ON ref.tournamentIdList = :tournamentIdList AND ref.date = :date " +
                "WHERE ref.matchId = bean.matchId ORDER BY ref.`order` DESC limit 1"
    )
    abstract suspend fun queryEarlyLastMatch(tournamentIdList: List<Int>, date: Long): MatchBean?
    ...
    abstract suspend fun queryEarlyFirstMatch(tournamentIdList: List<Int>, date: Long): MatchBean?
    ...
    abstract suspend fun queryMatchChange(
        playType: Int,
        tournamentIdList: List<Int>
    ...
    @Transaction
    @Query(
        "SELECT ref.matchId as ownerMatchId, market.marketId as marketId, market.marketName as marketName, market.status as status, ref.selectionCount as defaultSelectionCount " +
    ...
    @Query(
        "SELECT ref.matchId as ownerMatchId, market.marketId as marketId, market.marketName as marketName, market.status as status, ref.selectionCount as defaultSelectionCount " +
                ...
                "WHERE market.marketId = ref.marketId ORDER BY ref.matchId, ref.`index` "
    ) // 按 matchId 排序有助於後續分組
    abstract suspend fun getMarketsForMatches(matchIds: List<Long>): List<MarketBeanLite>

    @Transaction
    @Query(
        "SELECT sel.selectionId as selectionId, " +
                ...
                "sel.odds odds," +
                ":oddsDisplayType as oddsDisplayType, " +
    ...
    abstract suspend fun getSelectionLites(
        matchId: Long,
        marketId: Long,
        oddsDisplayType: Int
    ): List<SelectionBeanLite>
    ...
    abstract suspend fun getOddSelectionsByIds(
        selectionIds: List<Long>,
    ): List<OldSelectionLite>
    ...
    open suspend fun insertMatch(
        ...
        if (isForce) {
            deleteCurrentTournamentMatchRef(playType, tournamentIdList, date)
        }
        return insertMatch(
    ...
    open suspend fun insertMatch(
        ...
        deleteMarketSelectionCrossRef(
            marketCrossRef.map { it.matchId },
            marketCrossRef.map { it.marketId })
    ...
    open suspend fun updateOnlyMatch(
        ...
        return getOneMatchByIds(updateIds, oddsDisplayType)
    }
    ...
    open suspend fun updateFullMatch(
        ...
        val oldOdds = getOddSelectionsByIds(selections.map { it.selectionId })
        insertMarkets(markets)
        insertSelections(selections)
        insertMatchMarketCrossRef(marketCrossRef)
        //盤口的selection有可能在推播時整個變更（例如兩個選項+0.5/-0.5 -> +1/-1），所以刪除之前的cross ref，把之前盤口和selection連結斷開再連接，避免query取得之前的盤口
        deleteMarketSelectionCrossRef(
            ...
            marketCrossRef.map { it.marketId })
        insertMarketSelectionCrossRef(marketSelectCrossRefs)
        return getOneMatchByIds(updateIds, oddsDisplayType).onEach {
            //加入賠率趨勢
    ...
    private fun specialHandling(
        marketId: Long,
        originSelections: List<SelectionBeanLite>
    ): List<SelectionBeanLite> {
...
//        } else { originSelections }
    ...
    @Transaction
    open suspend fun getOneMatchByIds(
        matchId: List<Long>,
        oddsDisplayType: Int
    ): List<MatchWithMarkets> {
        val matchBeans = getMatchByIds(matchId)
        val marketBeans = getMarketsForMatches(matchId)
        val selectionBeans = getSelectionLites(matchId, oddsDisplayType)

        return matchBeans.map { matchBean ->
            val markets =
                marketBeans.filter { marketBean -> marketBean.ownerMatchId == matchBean.matchId }
                    .map { marketBean ->
                        val selections =
                            selectionBeans.filter { selectionBean -> selectionBean.matchId == matchBean.matchId && selectionBean.marketId == marketBean.marketId }
    ...
    @Transaction
    open suspend fun getOneMatchById(
        matchId: Long,
        oddsDisplayType: Int
    ): MatchWithMarkets {
        return getMatchById(matchId).let { matchBean ->
            val markets = getMarkets(matchBean.matchId).map { marketBean ->
                val selections = specialHandling(
                    marketBean.marketId,
                    getSelectionLites(matchBean.matchId, marketBean.marketId, oddsDisplayType)
                )
                MarketWithSelections(marketBean, selections)
            }
            MatchWithMarkets(matchBean, markets)
