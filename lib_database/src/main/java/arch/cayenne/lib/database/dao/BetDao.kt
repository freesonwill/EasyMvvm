...
abstract class BetDao: BaseDao<BetBean>() {
    ...
    @Query("SELECT matchId, selectionId FROM BetSelectionBean WHERE betId = (" +
    ...
    abstract fun observeComboCount(status: BetStatusEnum = BetStatusEnum.PENDING, type: BetTypeEnum = BetTypeEnum.COMBO): Flow<Int>
