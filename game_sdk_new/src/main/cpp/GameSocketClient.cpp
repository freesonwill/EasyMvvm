...
)
{
    ...
    LOGD(">.>%s: mid=%u,sid=%u,text=[%s],textSize=[%d]", __FUNCTION__ ,mid, sid, str.data(), wDataSize);
...
extern "C"
JNIEXPORT jobjectArray JNICALL
Java_com_cn_game_sdk2_websocket_GameSocketClient_newUnpack(JNIEnv *env,jobject thiz,jbyteArray data){
    CCPayloadCipher *cipher = getChipper(env, thiz);
